...
## K. Sunlight (Ladybug + Gismo) and wind around trees and buildings (AeroField) (15 September)

**Study · sunlight** (`id solar`, `rec.solar`: the sky per month and the far horizon are saved, not the hours). Weather: Open-Meteo hourly `shortwave_radiation`, `direct_normal_irradiance`, `diffuse_radiation` for three full years in GMT, each value placed half an hour before its stamp (the mean of the preceding hour), or an EPW (fields 13–15, local standard time, hour ending). `sunAt()` is NOAA's general solar position. `slSky()` bins every hour on Tregenza's 145 patches: direct as a summed DNI vector per patch (so cos incidence is exact within a patch), hours with DNI > 120 W/m², diffuse and global totals per month; only years with ≥ 4,000 hours count. `slCompute()` per cell: horizon in 36 directions over the site (step grows after 16 cells, earth curve), plus Gismo's far horizon (`slFar()`: terrarium zoom 10 out to 25 km, the highest point per direction for the site's low, middle and high elevation); patch visible above the horizon; direct Σ max(0, n·D); isotropic diffuse DHI/K × Ω × cos (K = the patches' own Σ Ω sin alt, so flat open ground gets exactly DHI); 20% ground reflection × (1 − cos tilt)/2. Shows year, winter, summer, hours of direct sun; a spot reads the ground, a latitude-tilt panel (`slPanel`) and a monthly chart. ~2 s for the 3 km Encino site; fetch ~13 s.

Verified (`e2e239`, 13 checks): noon altitude at equinox/solstices within 0.6° of 90 − lat ± 23.44 and due south; a consistent synthetic year gives flat ground = Σ GHI exactly (2164.0); a 30° south slope within 0.05% of the hourly plane sum (2374.4 vs 2375.6); north slope December 32.6 vs south 155; a 25 m wall 20 m south cuts December to 24% and hours; the same year from an EPW equals; mocked far-hill tiles (a 2,500 m ring 8–14 km out) give 16.4° (16.7°) and cut winter sun. Real: Encino Solo open ground 2,010 kWh/m²/yr (ERA5 2023–2025), site 1,321–2,174, far hills only 1–3°.

**Wind · around trees and buildings** (`WF`, settings `treeD`, `hedgeH`, `flowRes`, `flowOn` in `rec.wind`; the flow itself is not saved). `wfLattice()` rotates the site so the wind blows along a lattice (padding 1.25 upwind, 3.6 × half-width across), buildings/objects/walls solid, crowns and hedges porous (drag 0.35/0.15/0.06). `WF_SRC` is a worker: D2Q9 BGK with Smagorinsky (Cs 0.17, τ0 0.51, u0 0.1), inflow equilibrium, outflow copy, periodic sides, full-way bounce-back, porous drag by the exact difference method, collide and stream fused with a precomputed neighbour table, 1.8 flow-throughs averaged over the last third; ~5–10 s at "quick". `wfLee()`: shelter behind each crown and hedge sample by distance in its height, Heisler and DeWalle-style curves (dense/medium/open, typical values), lowest where overlapping. `wfReady()` combines min(flow ratio, lee) with the flow direction; `wnVel`, the speed wash and a spot read use it. Point-cloud trees now store their measured `h`.
